...
namespace RollercoasterEdit
{
    public class TrackSegmentModify : MonoBehaviour
	{
        ...
        private bool isSupportsInvalid;
        private float supportRegnerateTime;
        private float meshGenerationTime;
        ...
        private void ResetMeshForTrackSegment(MeshGenerator meshGenerator, TrackSegment4 segment)
        {
            ...
            foreach(Transform child in segment.gameObject.transform) {
                ...
                if (mesh_filter != null) {
                    DestroyImmediate (mesh_filter.mesh);
        ...
        private void GenerateHeightMarkerTrack(TrackSegment4 trackSegment)
        {
            ...
            float sample = trackSegment.getLength(0) / Mathf.RoundToInt(trackSegment.getLength(0)/ .2f);
            float pos = 0.0f;
            int index = 0;
            ...
            LandPatch terrain = GameController.Instance.park.getTerrain(this.transform.position);
            Vector3 vector = position;
            if (terrain != null)
            {
                vector = terrain.getPoint(this.transform.position);
            }
